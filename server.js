...
async function fetchActiveListings(cardName, cardNumber, setTotal, token, isPromoOverride = false, stampVariant = 'none') {
  ...
  // Note: Japanese card name already has 'japanese' appended by the price route
  // Promo cards — adjust query based on stamp variant
  if (isPromoOverride || isPromo) {
    if (stampVariant === 'pokemon_center') {
      query = `${cardName} ${cardNumber} promo pokemon center stamp`;
      console.log('PC stamp promo query:', query);
    } else {
  ...
  items.forEach(item => {
    ...
    // Apply stamp filter — exclude/include based on variant requested
    const hasStamp = ['stamp', 'pokemon center', 'pc stamp', 'pokecenter'].some(k => title.toLowerCase().includes(k));
    if (stampVariant === 'pokemon_center' && !hasStamp) return; // only stamped
    if (stampVariant === 'none' && hasStamp) return;            // exclude stamped
...
app.get('/price', async (req, res) => {
  ...
  try {
    ...
    const searchName = isJapanese ? `${name} japanese` : name;
    const activeData = await fetchActiveListings(searchName, number || '', total || '', token, isPromoParam, stampVariant);
...
// TCGdex diagnostic endpoint — tries multiple URL formats
app.get('/jp-test', async (req, res) => {
  const results = {};
  const tests = [
    // Test 1: EN cards with name filter (known working format)
    ['en_name', 'https://api.tcgdex.net/v2/en/cards?name=umbreon'],
    // Test 2: JA cards no filter (just list all)
    ['ja_all',  'https://api.tcgdex.net/v2/ja/cards'],
    // Test 3: JA cards with name filter
    ['ja_name', 'https://api.tcgdex.net/v2/ja/cards?name=umbreon'],
    // Test 4: JA with different name (simpler)
    ['ja_pika',  'https://api.tcgdex.net/v2/ja/cards?name=pikachu'],
    // Test 5: specific known JA card
